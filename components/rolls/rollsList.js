...
export default function RollsList({ rolls, loadMoreRolls, name, theme }) {
  ...
  return (
    <div id="rollsList" ref={rollsListRef} className='rounded-bl overflow-auto' style={{ height: '45vh', border: `2px solid ${theme.tertiary}` }}>
      {rolls && Object.values(rolls).length ? Object.values(rolls).map((roll) => (
        <div key={`${roll.data.time}${roll.data.name}`} className='flex border items-center' style={{border: `2px solid ${theme.tertiary}`, backgroundColor: (theme.backgroundImage ? '' : theme.secondary)}}>
          <p className='p-1 pr-2 min-w-fit'>{roll.data.time}</p>
          <p className='p-1 pl-2 border-l min-h-full w-full' style={{backgroundColor: (theme.backgroundImage ? '' : theme.primary)}}>
      ...
      <div className='flex border p-1 items-center justify-center' style={{border: `2px solid ${theme.tertiary}`}}>
