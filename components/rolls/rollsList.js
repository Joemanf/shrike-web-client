...
export default function RollsList({ rolls, loadMoreRolls, name, theme }) {
  ...
  return (
    <div id="rollsList" ref={rollsListRef} className='rounded-bl overflow-auto' style={{ height: '45vh', border: `1px solid ${theme.tertiary}` }}>
      {rolls && Object.values(rolls).length ? Object.values(rolls).map((roll) => (
        <div key={`${roll.data.time}${roll.data.name}`} className='flex border items-center' style={{border: `1px solid ${theme.tertiary}`, backgroundColor: theme.secondary}}>
